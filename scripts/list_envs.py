...
All the environments are registered in the `franka_rl` extension. They start
...
with contextlib.suppress(ImportError):
    import franka_rl.tasks_experimental  # noqa: F401
...
def _format_presets(preset_map: dict | None) -> str:
    ...
    Args:
        preset_map: Mapping of :class:`~franka_rl.utils.preset_target.PresetTarget`
    ...
    if preset_map is None:
        return "(unavailable)"
    from franka_rl.utils.preset_target import PresetTarget
...
def main():
    """Print all environments registered in `franka_rl` extension."""
    ...
    if args_cli.show_presets:
        from franka_rl.utils.preset_cli import enumerate_task_presets
